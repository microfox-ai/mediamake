...
export const presetAgent = aiRouter
    ...
    .agent('/', async (ctx) => {
        ...
        if (intent.includes('QUERY')) {
            ...
            const queryType = prompt.includes('internal') ? 'internal-only' :
                prompt.includes('tag') ? 'by-tag' :
                    prompt.includes('id') ? 'by-id' : 'all';
            const result = await ctx.next.callAgent('/query', { queryType, targetId: prompt });

            if ((result as any).ok) {
                const data = (result as any).data;
                return {
                    code: `// Query Results:\n${JSON.stringify(data.presets, null, 2)}`,
                    metadata: { id: 'query-task', title: 'Query Complete', description: `Found ${data.count} presets.` }
                };
            }
            ...
            const scope = prompt.includes('change') ? 'changes' :
                prompt.includes('file') ? 'fs' :
                    prompt.includes('db') ? 'db' : 'all';
            ...
            if ((result as any).ok) {
                ...
                return {
                    ...
                    metadata: { id: 'indexing-task', title: 'Indexing Report', description: data.message }
                };
            } else {
    ...
    .actAsTool('/', {
        id: 'preset',
        name: 'Preset Agent',
        description: 'Main entry point. Routes to either Generation or Indexing of presets based on user intent.',
