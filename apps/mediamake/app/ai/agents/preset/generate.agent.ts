...
import { z } from 'zod';
import {
  PresetGeneratorInputSchema,
  PresetGeneratorOutputSchema,
} from './helpers/schema';
...
export const generateAgent = aiRouter
  ...
  .agent('/', async ctx => {
    ...
    ctx.response.writeMessageMetadata({
      text: '🔍 Researching component library...',
    });
    ...
    console.log(
      `[GENERATE] Step 1 Complete: Found ${ragItems.length} RAG results`,
    );

    // --- 2. ARCHITECTURAL DESIGN ---
    ctx.response.writeMessageMetadata({
      text: '🏗️ Architecting component structure...',
    });
    console.log('[GENERATE] Step 2: Architect started');
    let planResult = await ctx.next.callAgent('/architect', {
      prompt,
      ragResults: ragItems,
    });
    if (!(planResult as any).ok)
      throw new Error('Architecture planning failed');

    let plan = (planResult as any).data;
    console.log(
      `[GENERATE] Step 2 Complete: Plan created with ${plan.structure?.length || 0} components, ${plan.dependencies?.length || 0} dependencies`,
    );

    // --- 3. TECH LEAD REVIEW ---
    ctx.response.writeMessageMetadata({
      text: '🧐 Tech Lead reviewing plan...',
    });
    ...
    while (!reviewPassed && attempts < 2) {
      ...
      console.log(
        `[GENERATE] Tech Lead Review (Attempt ${attempts + 1}): Approved=${review.approved}`,
      );
      ...
      if (review.approved) {
        ...
        if (review.revisedPlan) {
          ...
          ctx.response.writeMessageMetadata({
            text: '✅ Plan approved with improvements.',
          });
        } else {
        ...
        ctx.response.writeMessageMetadata({
          text: `⚠️ Plan rejected: ${review.critique}. Re-architecting...`,
        });
    ...
    const maxCodingAttempts = 2; // Validation retry limit
    ...
    while (codingAttempts < maxCodingAttempts) {
      ctx.response.writeMessageMetadata({
        text: `💻 Coding preset (Attempt ${codingAttempts + 1})...`,
      });
      console.log(
        `[GENERATE] Coding Attempt ${codingAttempts + 1}/${maxCodingAttempts}`,
      );
      ...
      console.log(
        `[GENERATE] Code generated (${code.length} chars). Metadata: ${JSON.stringify(generatedMeta)}`,
      );

      // Validate the code (validator will handle temp file creation and cleanup)
      ctx.response.writeMessageMetadata({
        text: '🛡️ Validating code (TypeScript, structure, ESLint)...',
      });
      ...
      if ((validatorResult as any).ok && (validatorResult as any).data.valid) {
        ...
        const wasAutoFixed =
          (validatorResult as any).data.wasAutoFixed || false;
        ...
        if (wasAutoFixed && fixedCode) {
          ...
          code = fixedCode;
          ctx.response.writeMessageMetadata({
            text: '✨ Code auto-fixed by ESLint',
          });
        }
        ...
        if (warnings.length > 0) {
          ...
          ctx.response.writeMessageMetadata({
            text: `✅ Validation passed (${warnings.length} warning${warnings.length > 1 ? 's' : ''})`,
          });
        } else {
          ctx.response.writeMessageMetadata({
      ...
      const errors = (validatorResult as any).data.errors || [
        'Unknown validation error',
      ];
      const lintOutput = (validatorResult as any).data.lintOutput || '';
      console.warn('[GENERATE] Validation Errors:', errors);
      ctx.response.writeMessageMetadata({
        text: `❌ Validation failed (${errors.length} error${errors.length > 1 ? 's' : ''}). Fixing & retrying...`,
      });
      lastErrors = errors;
    ...
    if (!success) {
      ...
      console.warn(
        `[GENERATE] Validation failed after ${maxCodingAttempts} attempts. Saving code with error metadata.`,
      );
  ...
  .actAsTool('/', {
    ...
    description:
      'Executes the preset generation workflow (Plan -> Code -> Save).',
