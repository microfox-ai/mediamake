  ...
  useEffect(() => {
    ...
    const generateThumbnail = async () => {
      try {
        ...
        // Create mediabunny input from URL
        input = new Input({
          formats: ALL_FORMATS,
        ...
        // Mediabunny's track types don't guarantee a `duration` field.
        // Use the public `input.computeDuration()` API to clamp the capture time.
        ...
        let t = timeInSeconds;
        if (duration !== null) {
          // Clamp capture time to [0, duration)
        ...
        // Normalise OffscreenCanvas vs HTMLCanvasElement
        ...
        if (canvas instanceof HTMLCanvasElement) {
          ...
          // OffscreenCanvas has a transferToImageBitmap method; draw it if available.
          // Fall back to casting as any to use drawImage.
        ...
        if (input && typeof (input as any).dispose === "function") {
          try {
