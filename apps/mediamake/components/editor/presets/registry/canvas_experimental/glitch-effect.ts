...
const presetParams = z.object({
    ...
    glitchType: z.enum(['rgb-shift', 'slice', 'corrupt', 'static', 'scan']).default('rgb-shift'),
    ...
    glitchStartTime: z.number().default(0).describe('When to start glitching (seconds from start).'),
    glitchEndTime: z.number().default(-1).describe('When to end glitching (seconds, -1 = full duration).'),
...
const presetExecution = async (
    ...
    const { trackId, imageUrl, start, duration, glitchType, intensity,
        frequency, continuous, glitchStartTime, glitchEndTime, backgroundColor, fit } = params;
    ...
    const component = {
        ...
        data: {
            ...
            glitchEndFrame: glitchEndTime === -1 ? -1 : Math.round(glitchEndTime * (fps ?? 30)),
