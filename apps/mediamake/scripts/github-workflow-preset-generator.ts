...
function parseIssueBody(issueBody: string): PresetRequest[] {
  try {
    ...
    console.log(
      '📄 Raw issue body preview:',
      jsonString.substring(0, 150).replace(/\n/g, ' ') + '...',
    );
    ...
    if (
      ...
      throw new Error(
        'Could not find JSON array in issue body (missing [ or ])',
      );
    }
...
function generatePRBody(
  ...
  const presetsWithValidationFailures = results.filter(
    r => r.success && r.validationFailed,
  );
  ...
  const presetsWithLintIssues = results.filter(
    r => r.success && r.hasLintIssues && !r.validationFailed,
  );
  ...
  try {
    ...
    try {
      ...
      if (execError.killed || execError.signal === 'SIGTERM') {
        throw new Error(
          `Generation timed out after ${GENERATION_TIMEOUT / 1000 / 60} minutes`,
        );
      }
    ...
    if (jsonStartIndex === -1 || jsonEndIndex === -1) {
      throw new Error(
        'Script output missing JSON markers. Unable to parse result.',
      );
    }
    ...
    return {
      ...
      hasLintIssues:
        (result.metadata.lintOutput &&
          (result.metadata.lintOutput.includes('error') ||
            result.metadata.lintOutput.includes('warning'))) ||
        false,
      originalRequest: request,
    };
  } catch (error: any) {
    console.error(
      `❌ [${index}/${totalCount}] Generation failed:`,
      error.message,
    );
    ...
    if (isRetryableError && retryCount < MAX_RETRIES) {
      ...
      console.log(
        `🔄 Retrying in ${waitTime / 1000} seconds... (Attempt ${retryCount + 1}/${MAX_RETRIES})`,
      );
  ...
  try {
    ...
    if (!fs.existsSync(relativeFilePath)) {
      ...
      if (fs.existsSync(alternatePath)) {
        ...
        throw new Error(
          `File not found: ${relativeFilePath} or ${alternatePath}`,
        );
      }
    }
...
async function main() {
  ...
  if (
    ...
    console.error(
      'Required: ISSUE_BODY, ISSUE_NUMBER, ISSUE_TITLE, GITHUB_TOKEN, GITHUB_REPOSITORY',
    );
    process.exit(1);
  }
  ...
  if (!issueTitle.toLowerCase().startsWith('new presets')) {
    console.log(
      '⏭️  Skipping: Issue title does not start with "new presets" (case-insensitive)',
    );
    process.exit(0);
  }
  ...
  for (let i = 0; i < presetRequests.length; i += BATCH_SIZE) {
    ...
    for (const result of batchResults) {
      if (result.success && result.filepath && result.metadata) {
        try {
          ...
          console.error(
            `   ⚠️  Failed to commit preset ${result.index}:`,
            error.message,
          );
    ...
    try {
      ...
      console.log(
        `✅ Updated PR: ${successCount}/${presetRequests.length} complete`,
      );
    } catch (error: any) {
  ...
  console.log(
    `📊 Results: ${successCount} successful, ${failureCount} failed (${step2Duration}s)`,
  );
  ...
  const finalPrBody = generatePRBody(
    issueNumber,
    allResults,
    presetRequests.length,
    true,
